...
public class SWEMonthlyClinicSummaryActivity extends AppCompatActivity {
    ...
    private void initView() {
        ...
        createMonthlyClinicSummaryQuestionaire();

        if(SWEMonthlyClinicSummaryActivity.interchanges!=null){
            recyclerView = (RecyclerView) findViewById(R.id.recyclerView);
            recyclerView.setLayoutManager(new LinearLayoutManager(this));
            adapter = new InterchangeCardAdapter(SWEMonthlyClinicSummaryActivity.this, SWEMonthlyClinicSummaryActivity.interchanges);
    ...
    private void createMonthlyClinicSummaryQuestionaire() {
        try{
            ArrayList<Interchange> returnedInterchanges = BwfSurveyAmplifyApplication.getInterchanges("SWECLINICSUMMARY");
            if(returnedInterchanges!=null){
                SWEMonthlyClinicSummaryActivity.interchanges = new ArrayList<>();
                ...
                for(Interchange interchange : returnedInterchanges){
                    ...
                    SWEMonthlyClinicSummaryActivity.interchanges.add(interchange);
                    positionOnRecyler += 1;
                }
                //sort the interchanges
                Collections.sort(SWEMonthlyClinicSummaryActivity.interchanges);
            }
        }catch (Exception c){
            Log.i("Tutorial", "we cannot get list of interchanges " );
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.save) {
            ...
            if(invalideInterchanges.size()>0){
                InterchangeUtils.showInvalidSurveyAlert(invalideInterchanges,SWEMonthlyClinicSummaryActivity.this);
            }else{
                String lat_ = "";
                String lng_ = "";
                if(lat!=null&&lng!=null){
                    ...
                    PhoneLocation phoneLocation = new PhoneLocation(SWEMonthlyClinicSummaryActivity.this);
                ...
                //make an SWEMonthlySchoolSummary object
                SWEMonthlyClinicSummary sweMonthlyClinicSummary = makeSWEMonthlyClinicSummaryObject(interchangesWithUserAns,1,lat_,lng_);
                //save the SWEMonthlySchoolSummary object
                saveSWEMonthlyClinicSummary(sweMonthlyClinicSummary);
    ...
    private SWEMonthlyClinicSummary makeSWEMonthlyClinicSummaryObject(ArrayList<Interchange> interchangesWithUserAns,int completed, String lat, String lng) {
        ...
        SWEMonthlyClinicSummary sweMonthlyClinicSummary = SWEMonthlyClinicSummary.builder()
                ...
                .date(date)
                .build();
        return sweMonthlyClinicSummary;
    }

    SubscriptionToken checkToken = null;
    private void saveSWEMonthlyClinicSummary(SWEMonthlyClinicSummary sweMonthlyClinicSummary) {
        checkToken = Amplify.Hub.subscribe(
                ...
                hubEvent -> {
                    ...
                    if(event!=null && event.getModelName().contentEquals("SWEMonthlyClinicSummary")){
                        if(event.getElement().getModel().getId().contentEquals(sweMonthlyClinicSummary.getId())){
                            ...
                            runOnUiThread(new Runnable() {
                                public void run() {
                                    ...
                                    for(Interchange interchange: SWEMonthlyClinicSummaryActivity.interchanges){
                                        interchange.getAnswer().setAns(null);
                                    }
                                    SWEMonthlyClinicSummaryActivity.this.finish();
                                }
                            });
                        }
                    }else{
        ...
        Amplify.DataStore.save(sweMonthlyClinicSummary,
                update -> {
    ...
    private void showSavedSuccessfulAlert(){
        ...
        new AlertDialog.Builder(SWEMonthlyClinicSummaryActivity.this)
                .setTitle("Saved Succussfully")
                .setMessage("Monthly Clinic Summary Saved Succussfully \n" )
        ...
        runOnUiThread(new Runnable() {
            public void run() {
                new AlertDialog.Builder(SWEMonthlyClinicSummaryActivity.this)
                        ...
                        .setMessage("Monthly Clinic Summary Save Failed! Please try again\n" )
                        // A null listener allows the button to dismiss the dialog and take no further action.
