...
public class ViewSWEMonthlySchoolSummaryActivity extends AppCompatActivity {

    SWEMonthlySchoolSummary theSWEMonthlySchoolSummary;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getIntent().getStringExtra("UUID")!=null)
            uuidSWEMonthlySchoolSummary = getIntent().getStringExtra("UUID");
    ...
    private void initView() {
        ...
        createSWEMonthlySchoolSummaryViewOnlyInterchanges();
    }

    private void createSWEMonthlySchoolSummaryViewOnlyInterchanges() {
    ...
    private void createViewOnlyInterchangesAndShowOnRecyclerView() {
        Amplify.DataStore.query(
                ...
                Where.matches(SWEMonthlySchoolSummary.ID.eq(uuidSWEMonthlySchoolSummary)),
                sweMonthlySchoolSummary -> {
                    Log.i("Tutorials", "DataStore is queried.");
                    while (sweMonthlySchoolSummary.hasNext()) {
                        theSWEMonthlySchoolSummary = sweMonthlySchoolSummary.next();
                        Log.i("Tutorials", "DataStore is queried. theSWEMonthlySchoolSummary " +theSWEMonthlySchoolSummary.getId());
                    }
    ...
    private void createViewOnlyInterchangesAndShow() {
        if(theSWEMonthlySchoolSummary!=null){
            ...
            if(returnedInterchanges != null){
                for(int i=0;i<returnedInterchanges.size();i++){
                    ...
                    try {
                        method = theSWEMonthlySchoolSummary.getClass().getMethod(methodName);
                        Object ansObject = method.invoke(theSWEMonthlySchoolSummary);
                        if(ansObject!=null)
    ...
    private void initViewElements() {
        ...
        adapter = new ViewOnlyInterchangeCardAdapter(ViewSWEMonthlySchoolSummaryActivity.this, ViewSWEMonthlySchoolSummaryActivity.viewOnlyInterchanges);
