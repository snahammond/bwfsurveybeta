...
public class ViewSWEMonthlyClinicSummaryActivity extends AppCompatActivity {
    SWEMonthlyClinicSummary theSWEMonthlyClinicSummary;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getIntent().getStringExtra("UUID")!=null)
            uuidSWEMonthlyClinicSummary = getIntent().getStringExtra("UUID");
    ...
    private void initView() {
        ...
        createSWEMonthlyClinicSummaryViewOnlyInterchanges();
    }

    private void createSWEMonthlyClinicSummaryViewOnlyInterchanges() {
    ...
    private void createViewOnlyInterchangesAndShowOnRecyclerView() {
        Amplify.DataStore.query(
                ...
                Where.matches(SWEMonthlyClinicSummary.ID.eq(uuidSWEMonthlyClinicSummary)),
                sweMonthlyClinicSummary -> {
                    Log.i("Tutorials", "DataStore is queried.");
                    while (sweMonthlyClinicSummary.hasNext()) {
                        theSWEMonthlyClinicSummary = sweMonthlyClinicSummary.next();
                        Log.i("Tutorials", "DataStore is queried. theSWEMonthlyClinicSummary " +theSWEMonthlyClinicSummary.getId());
                    }

                    runOnUiThread(() -> createViewOnlyInterchangesAndShow());
                },
                failure ->{
    ...
    private void createViewOnlyInterchangesAndShow() {
        if(theSWEMonthlyClinicSummary!=null){
            ...
            if(returnedInterchanges != null){
                for(int i=0;i<returnedInterchanges.size();i++){
                    ...
                    try {
                        method = theSWEMonthlyClinicSummary.getClass().getMethod(methodName);
                        Object ansObject = method.invoke(theSWEMonthlyClinicSummary);
                        if(ansObject!=null)
    ...
    private void initViewElements() {
        ...
        adapter = new ViewOnlyInterchangeCardAdapter(ViewSWEMonthlyClinicSummaryActivity.this, ViewSWEMonthlyClinicSummaryActivity.viewOnlyInterchanges);
